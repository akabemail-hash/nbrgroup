...
import { ProductInsert, ProductInsertSubmission } from '../types';
import { Loader2, Search, Calendar, DollarSign, CheckCircle, Circle, Save, X } from 'lucide-react';
                ...
                <div className="overflow-y-auto p-6 space-y-6">
                    ...
                    <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-lg border border-border dark:border-dark-border space-y-2">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                <p className="text-xs text-text-secondary uppercase">{t('productInsert.productName')}</p>
                                <p className="font-medium">{insert.product_name}</p>
                            </div>
                            <div>
                                <p className="text-xs text-text-secondary uppercase">{t('productInsert.insertPrice')}</p>
                                <p className="font-medium text-green-600">{insert.insert_price.toFixed(2)}</p>
                            </div>
                            <div>
        ...
        try {
            ...
            if (activeVisit) {
                customerIds = [activeVisit.customerId];
                 ...
                 // Fallback if role is Admin or if logic somehow bypasses guard (unlikely for Seller/Merch)
                 // This part handles gathering ALL assigned customers if no specific visit is active (e.g. Admin view)
                if (role === 'Satış') {
            ...
            let query = supabase
                ...
                .select('*, customer:customers(name), submission:product_insert_submissions(*)')
            ...
            // Filter submissions to user specific if needed, though usually 1 submission per insert per user
            // The relationship 'submission' might return multiple if strictly joined, but usually 1:1 context here
            // We need to filter the submission array if it returns multiple to find ONE for CURRENT user
             const processedData = (data as any[]).map(insert => {
                 // submission might be an array or object depending on relationship. 
                 // Assuming strict 1:1 via join isn't perfect in Supabase-js without extra filters.
                 // Actually, let's just fetch submissions separately to be safe or rely on simple join.
                 // Ideally, we filter submission by user_id in the join, but Supabase simple join doesn't filter nested easily.
                 // For now, let's assume we filter on client if array.
                    ...
                    {inserts.map(insert => (
                        <div key={insert.id} className="bg-surface dark:bg-dark-surface rounded-xl shadow-md border border-border dark:border-dark-border overflow-hidden">
                            ...
                            <div className="p-4 space-y-3">
                                ...
                                <div className="flex items-center text-sm font-bold text-primary dark:text-dark-primary gap-2">
                                    <DollarSign className="h-4 w-4" />
                                    <span>{insert.insert_price.toFixed(2)}</span>
                                </div>
