...
-- MIGRATION: Ensure 'price' column exists in fixed_asset_brands
DO $$ 
BEGIN 
...
CREATE TABLE IF NOT EXISTS public.product_inserts (
  ...
  insert_price numeric(10, 2) NOT NULL DEFAULT 0,
...
-- MIGRATION: Add product_price to competitor_price_analysis_items if not present
DO $$ 
BEGIN 
...
-- MIGRATION: Ensure 'before_image_urls' and 'after_image_urls' column exists in product_shelves
DO $$ 
BEGIN 
...
BEGIN
  ...
  joined_data AS (
    SELECT
      ...
      sv.visit_datetime,
      sv.description,
      COALESCE(sv.is_visit, false) as visit_flag_internal,
  ...
  final_filtered AS (
    ...
    WHERE (filter_status = 'all')
       OR (filter_status = 'completed' AND completed = true AND visit_flag_internal = true)
       OR (filter_status = 'no_visit' AND completed = true AND visit_flag_internal = false)
  ...
  SELECT
    ...
    jd.completed::boolean,
    jd.visit_id::uuid,
    jd.visit_datetime::timestamptz,
    jd.description::text,
    jd.visit_flag_internal::boolean,
...
BEGIN
  ...
  joined_data AS (
    SELECT
      ...
      mv.visit_datetime,
      mv.description,
      COALESCE(mv.is_visit, false) as visit_flag_internal,
      nvr.name as no_visit_reason_text,
